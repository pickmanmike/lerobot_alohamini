...
A request that returns no newer observation sequence while the last fresh follower sample is younger than one second is transient: the client counts it, retains the last complete action, does not read a leader sample, and retries while the sender cadence continues. The freshness clock begins when the approved follower payload is received, before its paired leader read, and that exact timestamp crosses the startup/alignment gate into live forwarding. A later fresh observation accepts only one latest validated leader sample, without queued intermediate actions. At one second, staleness latches once, freezes the complete action for the remainder of a bounded run, and exits status `2`; an unbounded run stops promptly instead of holding stale forever. For each advancing sequence, the client retains the exact raw payload key set alongside its stable normalized observation schema, so normalization cannot hide a missing arm key by filling zero or hide an unexpected arm key by dropping it. Legitimate non-arm base/lift keys are ignored by the exact AM1 arm-key check. Partial, malformed, nonfinite, out-of-range, or regressing observations and invalid leader samples safety-refuse directly before another leader read. Every refusal stops and joins the sender before the main thread requests final zero and cleans up, preserving the original exception.
...
git merge-base --is-ancestor f686a6ad26d413be8c608fa774e94012b40394dc HEAD
if ($LASTEXITCODE -ne 0) { throw 'The reviewed Windows AR1 code commit is not present.' }
$reviewedTail = @(git diff --name-only f686a6ad26d413be8c608fa774e94012b40394dc..HEAD)
