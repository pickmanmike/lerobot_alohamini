...
**One combined session, not another standalone comparison:**
...
4. Apply power when ready, then run the existing Pi helper:
   ...
   Launching this helper authorizes immediate startup homing; there is no `RELIEF` prompt. Expect one slow downward home, a short stopped qualification, one controlled ~10 mm rise, then `operational_ready` at roughly 10–12 mm, zero goal velocity and torque on. Homing stops on detected bottom contact, or refuses on a fault, the 600 mm travel guard, or the 180-second deadline. Observe the entire descent and stop immediately for unexpected contact, direction or behavior. Wait for **both** `operational_ready` and successful host initialization/`Waiting for commands` before launching Windows. Do not continue after a refusal, wait at the bottom, or restart until a run happens to pass.
5. In a separate Windows terminal:
   ...
   Keep leaders still, follow the existing exact `SYNC` gate and post-sync Enter/alignment gate, and wait for `TELEOPERATION ACTIVE`. The existing helper permits at most 30 live seconds at 10 Hz. Check one small joint movement per arm, brief `W` then release, brief `U` then release above the 5 mm floor, brief `J` then release, and `Q`. Do not descend back against the lower hard stop. Base/lift stay zero during synchronization and pauses. Do not add camera or recording work.
6. Once motion has stopped, safely support the carriage/arms before pressing Ctrl+C in the Pi helper. Existing cleanup requests zero/torque-off and records a bounded grouped stopped/off verification before closing the bus. Remove motor power. A real fault follows the same stop/cleanup path; do not reach into a moving mechanism—use the accessible disconnect and prepared support. No automatic restart.
...
Pass: correct useful combined operation and release/Q stops; no live watchdog event or stale latch; stable raised rest with current below the 200 mA refusal criterion; no confirmed sustained heating, real motor/transport/motion fault or unexpected physical behavior; successful stopped/torque-off cleanup and both exit codes 0. Known isolated numeric spikes with logged warnings are allowed. Stop immediately for any genuine fault/refusal, unsafe motion/load, heating/sound, failure to stop, power/USB issue or loss of support. Do not widen limits or automatically retry.
