...
This is a bounded electrical, feedback, direction, and unloaded-motion pass for the original servo and direct cable. It closes the comparator's endpoint-verdict defect and removes any reason to repeat the unloaded run or test the spare merely for confirmation. It does not resolve the earlier installed lower-stop heating, validate a loaded raised rest, or prove long-duration Local-mode thermal safety.

**Default next operator action:** run the installed-platform, ID-11-only grouped home/relief/rest comparison described below. Do not repeat the unloaded comparator first.

A coherent grouped high-temperature jump during the installed comparison would make scalar read association a weaker hypothesis and leave servo feedback/sensor, firmware, electrical behavior and actual loaded heating to distinguish. Stable grouped temperature with coherent position/velocity would instead make the earlier scalar/homing transaction timing or lower-stop load path more plausible, without proving either. A grouped-read preflight failure identifies vendor sync-read compatibility or the transport boundary before motion and authorizes no fallback motion. External touch temperature cannot validate the embedded sensor. Preserve the automatic `/home/pickmanmike/AlohaMini1Logs/am1-lift-host-YYYYMMDD-HHMMSS.log` for review.
...
**Installed comparison starting state and command:** Stage the exact reviewed PR #5 head with power off and a clean Pi checkout first.
...
This comparison asks whether the installed, normally loaded lift produces coherent grouped feedback and remains cool and low-current after leaving the lower stop for a short raised rest. A short pass would support—but not uniquely prove—lower-stop loading as the discriminator. It is not long-duration Local-mode thermal closure. Persistent elevated raised current or another anomaly refuses the check and calls for evidence review, not higher limits, another long settle, a PID change or another baseline. PR #5 stays draft/unmerged. Cameras, recording, autonomy, battery work, Cloudflare and remote operation remain out of scope and unproven.
