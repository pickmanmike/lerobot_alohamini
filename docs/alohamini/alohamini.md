...
The following single packet is preparation for a separately authorized human-operated check of lift resting condition and repaired shutdown. It is not authorization supplied by this guide, and it deliberately repeats no arm, base, or lift-direction movement.

Starting state: the Pi host is stopped; both leader supplies are off and their USB controllers disconnected; the chassis is fixed; the follower arms are supported; the unloaded lift has a clear homing envelope; body power is initially off; and the 12 V disconnect is immediately accessible. Use a clean checkout of the reviewed PR head. Apply body power and run only:

```bash
./tools/run_am1_host.sh --mode lift --lift-diagnostics
```

Require the single slow physical-down homing cycle to reach the real bottom, stop promptly, and enter a quiet hold. Do not start a Windows client and do not command any movement. Starting with the first post-home `[LIFT DIAGNOSTICS]` record, observe for up to 10 minutes. Stop early with Ctrl+C for any status other than `0`, goal velocity other than `0`, sustained present velocity, any sample at or above 300 mA, three consecutive samples at or above 200 mA, a steadily rising temperature accompanied by elevated current, unstable voltage, communication error, unexpected motion, heat, sound, vibration, or loss of the disconnect path. These are test refusal criteria, not changed motor limits. Otherwise, after 10 minutes press Ctrl+C once, wait for cleanup and `HOST_EXIT_CODE`, then remove body power.

Pass requires one successful home; no post-home motion; goal velocity `0`; present velocity settled at or near `0`; torque/mode/status remaining `1/1/0`; current staying outside the earlier high-current band; temperature and voltage remaining stable; no overheat or missing ID; final zero and torque-off without `Port is in use!`; and `HOST_EXIT_CODE=0`. Preserve the automatic `/home/pickmanmike/AlohaMini1Logs/am1-lift-host-*.log`. A refusal authorizes no automatic retry or motor-setting change.

The smallest remaining human fact is: exactly what changed physically or electrically between the high-current attempt and the final low-current attempt—such as a complete body-power cycle, supply or wiring reseat/change, manually unloading or moving the lift, or something else? Until that is recorded and the packet above passes, combined local operation is demonstrated but not closed. Cameras, recording, autonomy, battery work, Cloudflare, and remote operation remain out of scope and unproven.
