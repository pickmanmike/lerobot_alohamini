...
Read the stationary grouped baseline and type `ROTATE` only if the source header, adapter alias, ID, configuration, temperature, current, voltage, torque-off state and clear physical envelope are correct. Expect one small rotation in the servo's positive raw direction, an automatic zero in about 0.3 seconds, and stopped grouped feedback; there is no endstop search or platform-height claim. Immediately remove motor power for unexpected direction or movement, unusual sound or warmth, missing/faulted data, any temperature/current/status refusal, failure to stop, cleanup failure or any exit other than `LIFT_MOTOR_FEEDBACK_PASS` with `HOST_EXIT_CODE=0`. Do not retry a refusal.
...
**Deferred relief starting state and command:** Do not use this unless the independent grouped motor-feedback result above is reviewed and specifically supports returning to the relief comparison. Stage the reviewed PR #5 head with power off and a clean Pi checkout first. Leave leader power off and USB disconnected, stop every motor host, fix the chassis and secure the inactive arms without blocking carriage travel or offloading its mass. Use the normal carriage mass with no added payload; do not hold or artificially unload the lift during measurement. Clear the homing/relief path and prepare safe carriage support for the automatic torque-off at success or fault, without putting hands in pinch points. Let the servo cool fully; an externally warm enclosure is not evidence that internal temperature is safe. Make motor-power removal immediately accessible. Only a separately authorized human run may use, from the Pi terminal:
