...
Normal AM1 host startup (both `--mode local` and `--mode lift`, not just `--lift-relief`) now uses grouped lift feedback on the existing left-bus owner. It proves five genuine cool/stationary readings before activation, homes once, verifies stopped zero, raises approximately 10 mm at the established logical `+200` (raw `-200`), and stops with torque **enabled** clear of the hard stop. The existing 12 mm relief bound, 8-second relief timeout, 20-second homing timeout, original process-local zero and real 5 mm descent floor remain. There is no extended bottom dwell, automatic retry, or replacement home zero.
...
3. With the host stopped, fast-forward the clean Pi checkout of `feature/am1-local-mode` to the exact reviewed SHA supplied with this packet. Verify the printed SHA before enabling power. No package installation is needed. Use the actual Windows Local worktree and existing ignored Base-worktree config; do not copy calibration files.
4. Apply power when ready, then run the existing Pi helper:
   ...
   Expect one slow downward home, a short stopped qualification, one controlled ~10 mm rise, then `operational_ready` at roughly 10–12 mm, zero goal velocity and torque on. Do not continue if this sequence refuses or physical motion is unexpected. Do not wait at the bottom or restart until a run happens to pass.
