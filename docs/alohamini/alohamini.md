...
- Start at or below **40 C**, torque off, zero goal velocity, usable stationary telemetry and status zero. Exact uppercase `RELIEF` authorizes the whole bounded sequence. Fresh checks follow the gate and immediately precede torque activation; the gate never waits at the powered lower stop.
...
- Every exit requests existing zero/torque-off/bus-close cleanup. Only this diagnostic shields that bounded cleanup from a second Ctrl+C, restoring the prior signal handler afterward. A primary refusal remains primary if cleanup also fails. Exit `0` means the bounded comparison and cleanup completed, `2` is diagnostic refusal, `130` operator interruption, and `1` an unexpected I/O/runtime or cleanup failure. No status authorizes an automatic repeat.
