...
**Next action is torque-off readback, not relief.** With the host stopped, all motor power initially off, leaders off/disconnected, the chassis fixed and the carriage safely supported, stage the reviewed PR #5 head. Make motor-power removal immediately accessible, apply body power, and run only:

```bash
cd /home/pickmanmike/lerobot_alohamini && ./tools/run_am1_host.sh --mode lift --lift-readback
```

This reads configuration once and then records three seconds of fresh, torque-off, stationary telemetry. It sends no torque-enable, homing or nonzero velocity command. Any visible motion, abnormal sound, warmth, missing/faulted reply, pending response, nonzero goal velocity, present-velocity magnitude above the established stationary threshold of 5 raw units, torque-on state, status fault, value at or above 55 C, or exit other than the expected bounded result requires immediate power removal and evidence review without retry. A pass requires no motion, `LIFT_READBACK_PASS`, verified cleanup and `HOST_EXIT_CODE=0`; preserve the automatic `/home/pickmanmike/AlohaMini1Logs/am1-lift-host-YYYYMMDD-HHMMSS.log` and stop. A pass still does not authorize relief automatically; review the exact trace first.

Fixed diagnostic policies (not manufacturer ratings or changed servo settings):

- Start at or below **40 C**, torque off, zero goal velocity, usable stationary telemetry and status zero. Exact uppercase `RELIEF` authorizes the whole bounded sequence. Fresh checks follow the gate. Diagnostic-only `setup_before` and `setup_after_writes` records bracket the existing lift setup; after every mode/zero/lock setup write, a direct final torque-off request must read back torque `0`, goal velocity `0`, mode `1` and stationary telemetry in `before_torque`. No other setup write occurs before intentional activation. The gate never waits at the powered lower stop.
- Abort at **55 C**, or the existing temperature limit if lower, in **every** phase. Missing/faulted/nonfinite telemetry, a telemetry read/gap over 0.5 s, invalid torque/mode/goal readback, unexpected direction or excessive velocity also refuse. Reported voltage must remain within the limits actually read from the servo; the operator must also stop for visible supply instability.
- Call the **existing homing once**, raw `+200` physically down, retaining its 20-second timeout and stall detection. An optional callback checks telemetry and upward-positive homing displacement at every poll without consuming the homing position accumulator. It rejects a nonfinite initial position before torque and late completion after telemetry latency. Homing travel cannot exceed the existing 600 mm travel envelope. Failed/nonfinite current reads cannot use the ordinary fallback while this callback is active; ordinary operation is unchanged.
- After one immediate post-home snapshot, request logical **`+200`** (raw `-200`) physically up to **10 mm**. No second operator pause at the bottom. Refuse wrong direction, less than 0.5 mm progress after 2 s, failure to reach the target within 8 s, or travel beyond 12 mm. Command zero as soon as 10 mm is measured.
- Allow at most 1 s to settle, requiring three stationary samples, then observe raised rest for **45 s**. Refuse loss of relief below 9.5 mm, more than 0.5 mm stationary drift, or nonstationary velocity. The near-zero velocity threshold is 5 raw units; this does not replace the visible-stop requirement.
- During rest, **three consecutive samples at or above 200 mA** abort. Checks run every 0.1 s plus bounded serial-read time; routine phase telemetry prints at roughly 1 Hz, and all three elevated-current samples are recorded. Homing and relief/settling transients do not use the idle-current criterion, but retain their temperature, motion, travel and finite-time bounds.
...
**Deferred relief starting state and command:** Do not use this until the torque-off readback above passes and its exact evidence receives review. Stage the reviewed PR #5 head with power off and a clean Pi checkout first. Leave leader power off and USB disconnected, stop every motor host, fix the chassis and secure the inactive arms without blocking carriage travel or offloading its mass. Use the normal carriage mass with no added payload; do not hold or artificially unload the lift during measurement. Clear the homing/relief path and prepare safe carriage support for the automatic torque-off at success or fault, without putting hands in pinch points. Let the servo cool fully; an externally warm enclosure is not evidence that internal temperature is safe. Make motor-power removal immediately accessible. Only the human then applies body power and runs, from the Pi terminal:

```bash
cd /home/pickmanmike/lerobot_alohamini && ./tools/run_am1_host.sh --mode lift --lift-relief
```

Read the cold preflight and type `RELIEF` only if ready. Observe one slow down/home, immediate upward relief of approximately 10 mm, stop, and at most 45 seconds stationary observation. Require the home to reach the actual bottom, correct upward relief, visibly stopped raised carriage, no wheel/arm movement, and no concerning sound, heating, contact or supply behavior. Any contrary observation: Ctrl+C once, safely support the carriage and remove motor power; do not wait for repeated cleanup attempts when communication has failed. Automatic faults already enter cleanup, so do not restart them. On normal completion, confirm `LIFT_RELIEF_PASS` and `HOST_EXIT_CODE=0`, safely support as needed and remove motor power. Keep the original automatic `/home/pickmanmike/AlohaMini1Logs/am1-lift-host-YYYYMMDD-HHMMSS.log`; its header now records branch, exact SHA and launch command, and phase records include the home result and transient/rest measurements. No Windows helper is needed.

A short cool, low-current raised rest would support—but not uniquely prove—lower-stop loading as the discriminator. Persistent elevated raised current refuses this check and calls for evidence review, not higher limits, another long settle, a PID change or another baseline. PR #5 stays draft/unmerged. Cameras, recording, autonomy, battery work, Cloudflare and remote operation remain out of scope and unproven.
